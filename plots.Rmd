...
mofo1$fungicide2 <- revalue(mofo1$fungicide2, c("TIOF_4" = "TIOF"))
...
mofo1 <- mofo1 %>% 
        mutate(inc_class_1 = ifelse(inc_check < 30, "low","high"),
...
ggsave("Figures/Fig1_supp.png", width = 8, height=11, dpi = 300)
```
...
mofo1$fungicide2 <- factor(mofo1$fungicide2, levels = c("CHECK", "CARB+PROC", "DIMO+BOSC", "FLUO", "FLUZ",  "PROC", "TIOF"))
...
ggsave("Figures/Fig4.png", width = 9, height=4, dpi = 300)
```
##  Yield (yld)
...
ggsave("Figures/Fig6.png", width = 9, height=4, dpi = 300)
```
...
ggsave("Figures/Fig3.png", width = 8, height=4, dpi = 300)
```

# Efficacy of fungicides


## Incidence x Sclerotia Mass

```{r}
library(ggrepel)
scl_inc <- gsheet2tbl(
  "https://docs.google.com/spreadsheets/d/1VLK94nZV9DhYNVEUXFb7aC5BjhJ21FYfLf8crj5m-a8/edit#gid=41911812", sheetid = "inc_scl")
scl_inc <- scl_inc %>% 
  mutate(inc = as.numeric(inc),
         scl = as.numeric(scl),
...
geom_label_repel(aes(inc, scl, label = scl_inc$Fungicide), fill = "grey90",  size=3, color = 'black',label.size=0.30,
...
ggsave("Figures/Fig5.png",width=5, height=4, dpi=300)
...
library(tidyverse)
inc_yld <- gsheet2tbl(
  "https://docs.google.com/spreadsheets/d/1VLK94nZV9DhYNVEUXFb7aC5BjhJ21FYfLf8crj5m-a8/edit#gid=2054312057", sheetid = "inc_yld")
inc_yld <- inc_yld %>% 
  mutate(inc = as.numeric(inc),
         yld = as.numeric(yld),
         inc_inf = as.numeric(inc_inf),
...
gr1 <- ggplot(inc_yld,aes(inc, yld)) +
  ...
  geom_label_repel(aes(inc, yld, label = inc_yld$Fungicide), fill = "grey90",
...
## Sclerotia Mass x Yield


```{r}
scl_yld <- gsheet2tbl(
  "https://docs.google.com/spreadsheets/d/1VLK94nZV9DhYNVEUXFb7aC5BjhJ21FYfLf8crj5m-a8/edit#gid=1324985291", sheetid = "scl_yld")
scl_yld <- scl_yld %>%
  mutate(yld = as.numeric(yld),
         scl = as.numeric(scl),
         yld_inf = as.numeric(yld_inf),
         yld_sup = as.numeric(yld_sup),
         scl_inf = as.numeric(scl_inf),
...
geom_label_repel(aes(scl, yld, label = scl_yld$Fungicide), fill =  "grey90",  size=3.5, color = 'black',label.size=0.50,
    box.padding = unit(0.30, "lines")) +
...
ggsave("Figures/Fig7.png", width = 9, height=5, dpi = 300)
...
mapa<- gsheet2tbl(
  "https://docs.google.com/spreadsheets/d/1VLK94nZV9DhYNVEUXFb7aC5BjhJ21FYfLf8crj5m-a8/edit#gid=1595668142", sheetid = "Map")
mapa <- mapa %>% 
...
mapa <- ggplot(mapa) + 
  ...
  geom_jitter(alpha = 1/2, data = mapa, aes(x = lon, y = lat, 
                                  size = inc, 
                                  colour = NULL))+
  coord_fixed()+
  theme_minimal()+
 ...
 mapa
ggsave ("Figures/Fig1.png", width=6, height=6,  dpi = 300)
...
mofo <- gsheet2tbl(
  "https://docs.google.com/spreadsheets/d/1VLK94nZV9DhYNVEUXFb7aC5BjhJ21FYfLf8crj5m-a8/edit#gid=2085223602", sheetid = "Probability")
mofo <- mofo %>% 
...
library(ggthemes)
dat <- mofo %>% 
  mutate(breacking_even = as.numeric(breacking_even),
         breacking_even =round(breacking_even, 1))


dat %>%
 mutate(breacking_even1 = case_when(breacking_even >= 75 ~ "p \u2265 75  ",
...
theme_minimal()+
 ...
 theme(text = element_text(size = 12), legend.justification = "center", strip.text.x = element_text(size = 12, face ="bold"), strip.text.y = element_text(size=12, face="bold"), panel.grid.minor=element_line(colour = "white"),panel.grid.major=element_line(colour = "white"), legend.position = "bottom")
  ggsave("Figures/Fig8.png",width=8, height=7, dpi=300)
